...
/* Endpoint Buffer */
volatile __attribute__ ((aligned(4))) uint8_t USBFS_EP0_Buf[DEF_USBD_UEP0_SIZE];
volatile __attribute__ ((aligned(4))) uint8_t USBFS_EP2_Buf[DEF_USB_EP2_FS_SIZE];
...
void USBFS_Device_Endp_Init( void )
{
    ...
    USBFS->UEP0_DMA = (uint32_t)USBFS_EP0_Buf;
    USBFS->UEP1_DMA = (uint32_t)Data_Buffer;
    USBFS->UEP2_DMA = (uint32_t)USBFS_EP2_Buf;

    USBFS->UEP0_CTRL_H = USBFS_UEP_T_RES_NAK | USBFS_UEP_R_RES_ACK;
    USBFS->UEP1_CTRL_H = USBFS_UEP_R_RES_ACK;
...
void USBFS_Device_Init( FunctionalState sta , PWR_VDD VDD_Voltage)
{
    if( sta )
    {
        ...
        USBFS->DEV_ADDR = 0x00;
        USBFS->BASE_CTRL = USBFS_UC_DEV_PU_EN | USBFS_UC_INT_BUSY | USBFS_UC_DMA_EN;
        USBFS->INT_FG = 0xff;
        USBFS->UDEV_CTRL = USBFS_UD_PD_DIS | USBFS_UD_PORT_EN;
        USBFS->INT_EN = USBFS_UIE_SUSPEND | USBFS_UIE_BUS_RST | USBFS_UIE_TRANSFER;
...
void USBFS_IRQHandler( void )
{
    ...
    if( intflag & USBFS_UIF_TRANSFER )
    {
            ...
            case USBFS_UIS_TOKEN_IN:
                switch( intst & ( USBFS_UIS_TOKEN_MASK | USBFS_UIS_ENDP_MASK ) )
                {
                    ...
                    case USBFS_UIS_TOKEN_IN | DEF_UEP0:
                        ...
                        {
                            USBFS->UEP0_CTRL_H = (USBFS->UEP0_CTRL_H & ~ USBFS_UEP_R_RES_MASK) | USBFS_UEP_R_TOG | USBFS_UEP_R_RES_ACK;
                        }
            ...
            case USBFS_UIS_TOKEN_OUT:
                switch( intst & ( USBFS_UIS_TOKEN_MASK | USBFS_UIS_ENDP_MASK ) )
                {
                                    ...
                                    if (( USBFS_SetupReqType & USB_REQ_TYP_MASK ) == USB_REQ_TYP_CLASS)
                                    {
                                        switch( USBFS_SetupReqCode )
                                        {
                                            case HID_SET_REPORT:
                                                ...
                                                // ACK the STATUS stage immediately so EP0 is free for other requests
                                                USBFS->UEP0_TX_LEN = 0;
                                                USBFS->UEP0_CTRL_H = USBFS_UEP_T_TOG | USBFS_UEP_T_RES_ACK | USBFS_UEP_R_TOG | USBFS_UEP_R_RES_ACK;
                                                break;
                                            default:
                                                break;
                                        }
                    ...
                    case USBFS_UIS_TOKEN_OUT | DEF_UEP1:
                        /* Always accept EP1 OUT packets to bypass Windows HID toggle mismatch bug */
                        {
                            /* Write In Buffer */
                            USBFS->UEP1_CTRL_H ^= USBFS_UEP_R_TOG;
                            RingBuffer_Comm.PackLen[RingBuffer_Comm.LoadPtr] = USBFS->RX_LEN;
                            RingBuffer_Comm.LoadPtr ++;
                            if(RingBuffer_Comm.LoadPtr == DEF_Ring_Buffer_Max_Blks)
                            {
                                RingBuffer_Comm.LoadPtr = 0;
                            }
                            USBFS->UEP1_DMA = (uint32_t)(&Data_Buffer[(RingBuffer_Comm.LoadPtr) * DEF_USBD_FS_PACK_SIZE]);
                            RingBuffer_Comm.RemainPack ++;
                            if(RingBuffer_Comm.RemainPack >= DEF_Ring_Buffer_Max_Blks-DEF_RING_BUFFER_REMINE)
                            {
                                USBFS->UEP1_CTRL_H = (USBFS->UEP1_CTRL_H & ~USBFS_UEP_R_RES_MASK) | USBFS_UEP_R_RES_NAK;
            ...
            case USBFS_UIS_TOKEN_SETUP:
                    ...
                    switch( USBFS_SetupReqCode )
                    {
                        ...
                        case USB_SET_FEATURE:
                            ...
                            else if( ( USBFS_SetupReqType & USB_REQ_RECIP_MASK ) == USB_REQ_RECIP_ENDP )
                            {
                                ...
                                if( (uint8_t)( USBFS_SetupReqValue & 0xFF ) == USB_REQ_FEAT_ENDP_HALT )
                                {
                                    ...
                                    {
                                        case ( DEF_UEP_OUT | DEF_UEP1 ):
                                            USBFS->UEP1_CTRL_H = ( USBFS->UEP1_CTRL_H & ~USBFS_UEP_R_RES_MASK ) | USBFS_UEP_R_RES_STALL;
                                            break;
                                        case ( DEF_UEP_IN | DEF_UEP2 ):
                        ...
                        case USB_GET_STATUS:
                            ...
                            else if( ( USBFS_SetupReqType & USB_REQ_RECIP_MASK ) == USB_REQ_RECIP_ENDP )
                            {
                                ...
                                {
                                    if( ( USBFS->UEP1_CTRL_H & USBFS_UEP_R_RES_MASK ) == USBFS_UEP_R_RES_STALL )
                                    {
                ...
                else
                {
                    ...
                    if( USBFS_SetupReqType & DEF_UEP_IN )
                    {
                        ...
                        USBFS_SetupReqLen -= len;
                        USBFS->UEP0_TX_LEN = len;
                        USBFS->UEP0_CTRL_H = (USBFS->UEP0_CTRL_H & ~USBFS_UEP_T_RES_MASK) | USBFS_UEP_T_TOG | USBFS_UEP_T_RES_ACK;
                    }
                    else
                    {
                        if( USBFS_SetupReqLen == 0 )
        ...
        USBFS->INT_FG = USBFS_UIF_TRANSFER;
    }
    else if( intflag & USBFS_UIF_BUS_RST )
    {
        ...
        USBFS->DEV_ADDR = 0;
        USBFS_Device_Endp_Init( );
        USBFS->INT_FG = USBFS_UIF_BUS_RST;
    }
    else if( intflag & USBFS_UIF_SUSPEND )
    {
        USBFS->INT_FG = USBFS_UIF_SUSPEND;
        Delay_Us(10);
