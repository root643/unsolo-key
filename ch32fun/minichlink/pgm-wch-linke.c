...
static inline libusb_device_handle * wch_link_base_setup( int inhibit_startup, const char * want_serial )
{
	...
	// NOTE: DO NOT free the device list here! `found` / found_arm_programmer /
	// found_programmer_in_iap point INTO the list; freeing it with unref=1
	// before libusb_open() is a use-after-free that makes libusb_open fail
	// with LIBUSB_ERROR_NO_DEVICE (-4) depending on heap reuse.
	// The list is freed at every exit path below, after the opens.
