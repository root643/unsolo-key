#ifndef _FUNCONFIG_H
#define _FUNCONFIG_H

#define FUNCONF_USE_DEBUGPRINTF     0
#define FUNCONF_USE_UARTPRINTF      0
#define FUNCONF_ENABLE_HPE          0
#define FUNCONF_SYSTICK_USE_HCLK    1

#if defined(CH32X03x)
#define FUNCONF_USE_HSI             1
#define FUNCONF_USE_5V_VDD          0
#elif defined(CH5xx)
#define FUNCONF_ISR_IN_RAM          1
#define FUNCONF_USE_HSI             0 // CH5xx does not have HSI
#define CLK_SOURCE_CH5XX            CLK_SOURCE_PLL_24MHz // default so not really needed
#define FUNCONF_SYSTEM_CORE_CLOCK   24 * 1000 * 1000     // keep in line with CLK_SOURCE_CH5XX
#define FUNCONF_USE_CLK_SEC         0
#else
#define FUNCONF_USE_HSI             0
#define FUNCONF_USE_HSE             1
#endif

#define FUNCONF_DEBUG_HARDFAULT     1

#endif

