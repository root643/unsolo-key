...
int main(int argc, char **argv)
{
	...
	if( bRx )
	{
		while(!abortLoop)
		{
			...
			int nread, ret;
			ret = libusb_bulk_transfer( handle, EP_OUT, buffer, sizeof( buffer ), &nread, USB_TIMEOUT );
	...
	else
	{
		...
		while (!abortLoop)
		{
			...
			ret = libusb_bulk_transfer( handle, EP_IN, buffer, sizeof( buffer ), &nwrite, USB_TIMEOUT );
			double dNow = OGGetAbsoluteTime();
...
#elif defined( ASYNC_CONTROL ) // Async-control (Still slow, don't use, if you want this just use HIDAPI)
	...
	for( n = 0; n < TRANSFERS; n++ )
	{
		...
		struct libusb_transfer * t = transfers[n] = libusb_alloc_transfer( 0 );
		libusb_fill_bulk_transfer( t, handle, bRx ? EP_OUT : EP_IN, buffers[n], block_size, xcallback, (void*)(intptr_t)n, 1000 );
		libusb_submit_transfer( t );
	}
